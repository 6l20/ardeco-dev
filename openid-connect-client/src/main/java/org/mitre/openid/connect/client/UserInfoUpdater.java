...
/**
 * Utility class to update userinfo to the userinfo endpoint, if available.
 * @author jricher
 *
 */
public class UserInfoUpdater {
	 ...
	 */
	private static final Logger logger = LoggerFactory.getLogger(UserInfoUpdater.class);
	...
	private class UserInfoLoader extends CacheLoader<PendingOIDCAuthenticationToken, UserInfo> {
		...
		public UserInfo load(final PendingOIDCAuthenticationToken token) {
			...
			try {
				...
				logger.warn("Error updating userinfo", e);
				return null;
			}
